...
export default function ListItem({ label, icon, isSelect }: Props) {
  return (
    <li
      className={twMerge(
        'flex flex-1 min-w-fit stroke-neutral-gray-800 cursor-pointer h-9 button2 tablet:h-10 items-center gap-[6px] rounded-lg bg-neutral-gray-0 px-2 text-neutral-gray-800  tablet:px-[14px]',
        styles({ isSelect }),
      )}
    >
      {icon && <div>{icon}</div>}
